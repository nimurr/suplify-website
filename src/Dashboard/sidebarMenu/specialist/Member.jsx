...
//   const page = 1;
//   const limit = 10;
//   const { data: membersData, isLoading, isError, error } = useSpacialistAllPatentsQuery({page , limit});
//   const members = membersData?.data?.attributes?.results || [];
//   const totalPages = membersData?.data?.attributes?.totalPages || 1; 
...
const Members = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useSpacialistAllPatentsQuery({ page, limit });

  const members = membersData?.data?.attributes?.results || [];
  const totalItems = membersData?.data?.attributes?.totalResults || 0;
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          <Avatar
            size={32}
            src={url + record?.patient?.profileImage?.imageUrl}
          />
          <span className="font-semibold">{record?.patient?.name}</span>
        </div>
      ),
    ...
    },
    {
      ...
      render: (_, record) => (
        <span className="capitalize font-semibold">
          {record?.patient?.subscriptionType || "N/A"}
        </span>
      ),
    },
    {
      ...
      render: (_, record) => (
        <Link
          href={`/specialistDs/members/view-member?patientId=${record?.patient?._id}`}
