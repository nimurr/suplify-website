...
const HowItWorksPage = () => {
  ...
  const handleGetStarted = () => {
    if (users.role === 'patient') {
      navigate.push('/dashboard/doctor');
    }
    else if (users.role === 'specialist') {
      // window.location.href = '/specialistDs';
      navigate.push('/specialistDs/members');
    }
    else if (users.role === 'doctor') {
      // window.location.href = '/doctorDs';
      navigate.push('/dashboard/doctor');
    }
    else {
  ...
  return (
    <div className=" md:w-[70%] px-6 md:mt-12 mb-10 mx-auto p-6 bg-rose-50 rounded-lg shadow-md border">
      ...
      <div className="mt-10 text-center">
        <button onClick={handleGetStarted} className="px-6 py-3 bg-red-600 text-white rounded-lg font-medium shadow-md hover:bg-red-700 transition duration-200">
          Get Started Today
        </button>
      </div>
    </div>
