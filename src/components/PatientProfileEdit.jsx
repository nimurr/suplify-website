...
//                 externalLink: fullUser.externalLink,
...
//                 <Form.Item label="externalLink" name="externalLink">
//                     <Input className="h-10" placeholder="externalLink" />
//                 </Form.Item>
...
const PatientProfileEdit = () => {
    ...
    useEffect(() => {
        if (fullUser) {
            form.setFieldsValue({
                fullName: fullUser.name,
                email: fullUser.email,
                externalLink: fullUser?.profileId.externalLink || "",
                address: fullUser.profileId?.address || "",
                description: fullUser.profileId?.description || "",
                howManyPrograms: fullUser.profileId?.howManyPrograms || "",
                protocolNames: fullUser.profileId?.protocolNames || [],
            });
    ...
    const onFinish = async (values) => {
        ...
        formData.append("name", values.fullName);
        formData.append("email", values.email);
        if (imageFileList[0]?.originFileObj) formData.append("profileImage", imageFileList[0].originFileObj);
        formData.append("externalLink", values.externalLink || "");
        formData.append("address", values.address || "");
        formData.append("description", values.description || "");
        formData.append("howManyPrograms", values.howManyPrograms || "");
    ...
    const beforeImageUpload = (file) => {
        ...
        if (!isValid) message.error("You can only upload JPG/PNG files!");
        return isValid || Upload.LIST_IGNORE;
    };

    const handleImageChange = ({ fileList }) => setImageFileList(fileList.slice(-1));
    ...
    return (
        <div className="lg:w-[70%] mx-auto md:py-20 px-4 md:px-8">
            ...
            >
                ...
                {/* Protocol Names for Specialist */}
                {fullUser?.role === "specialist" && (
                    <div>
                        <h2 className="font-semibold mb-2">Protocol Names</h2>
                        <Form.List
                            name="protocolNames"
                            initialValue={fullUser?.profileId?.protocolNames?.length > 0 ? fullUser.profileId.protocolNames : [{ protocolName: "" }]}
                        >
                            {(fields, { add, remove }) => (
                                <>
